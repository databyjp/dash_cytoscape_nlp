...
for datafile in data_files:
    ...
    abs_text = ' '.join([i['text'] for i in data_dict['abstract']])  # Abstract could be in multiple sections
...
def vis_ngrams(docs_in, n_ngrams=20):
    ...
    unigram_df = pd.DataFrame([{'ngram': k, 'count': v} for k, v in frequencies.items() if '_' not in k]).sort_values('count', ascending=False)
    bi_gram_df = pd.DataFrame([{'ngram': k, 'count': v} for k, v in frequencies.items() if '_' in k]).sort_values('count', ascending=False)
    trigram_df = pd.DataFrame([{'ngram': k, 'count': v} for k, v in frequencies.items() if k.count('_') == 2]).sort_values('count', ascending=False)
    print(unigram_df[:n_ngrams])
    print(bi_gram_df[:n_ngrams])
    print(trigram_df[:n_ngrams])

    # Visualise counts of top n-grams
    fig = px.bar(unigram_df[:n_ngrams], x='ngram', y='count', title='Counts of top unigrams', template='plotly_white', labels={'ngram': 'Unigram', 'count': 'Count'})
    fig.show()
    fig = px.bar(bi_gram_df[:n_ngrams], x='ngram', y='count', title='Counts of top bi-grams', template='plotly_white', labels={'ngram': 'Bigram', 'count': 'Count'})
    fig.show()
    fig = px.bar(trigram_df[:n_ngrams], x='ngram', y='count', title='Counts of top trigrams', template='plotly_white', labels={'ngram': 'Trigram', 'count': 'Count'})
    fig.show()
    return True
...
# ========== Further filtering ==========
logger.info(f'Further cleaning the text by removing custom stopwords & rare/common words')
# Remove custom stopwords
custom_stopwords = [
    'play_important_role', 'play_critical_role', 'play_key_role', '95_confidence_interval',
    'provide_new_insight',
    'et_al', 'pubmed_abstract', 'publisher_text', 'present_study', 'results_suggest', 'result_suggest',
    '95_ci', 'play_important',
    'study', 'result', 'analysis', 'method'
]
docs = [[token for token in doc if token.lower() not in custom_stopwords] for doc in docs]
...
# ========== Visualise the data with plotly ==========
lda_vals = list()
for d in corpus:
    topics_tup = lda.get_document_topics(d)  # This should be a N by K matrix where N = corpus size, K = topics
...
for pplx in [55]:  # Test out different perplexity values
    lda_df = pd.DataFrame(lda_vals)
    lda_arr = deepcopy(lda_df.values)
    tsne_embeds = TSNE(n_components=2, perplexity=pplx, n_iter=3000).fit_transform(lda_arr)

    topics_txt = [lda.show_topics()[i][1].replace(' + ', '<BR>') for i in range(num_topics)]
    lda_df = lda_df.assign(topic_id=[str(lda_arr[i].argmax()) for i in range(len(lda_arr))])
    lda_df = lda_df.assign(topic_txt=['<BR>' + topics_txt[lda_arr[i].argmax()] for i in range(len(lda_arr))])
    ...
    fig = px.scatter(lda_df, title='t-SNE test, perplexity: ' + str(pplx),
                     x='x', y='y', color=topic_ids, color_discrete_sequence=px.colors.qualitative.Safe,
