...
network_df_sm = network_df[(network_df['cited_by'] != '') | (network_df['citations'] != '')]
old_indices = [str(i) for i in network_df_sm.index]
for i, row in network_df_sm.iterrows():
    rowloc = row.name
    network_df_sm.loc[rowloc, 'cited_by'] = ','.join([str(old_indices.index(j)) for j in network_df_sm.loc[rowloc, 'cited_by'].split(',') if len(j) > 0])
    network_df_sm.loc[rowloc, 'citations'] = ','.join([str(old_indices.index(j)) for j in network_df_sm.loc[rowloc, 'citations'].split(',') if len(j) > 0])
