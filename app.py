...
def filter_node_data(min_conns=5, journals=None, date_filter=None):
    # TODO - explore making this faster
    node_list_in = deepcopy(node_list)
    # temp_df = deepcopy(network_df)
...
def_stylesheet += [
            {
                ...
                'style': {'background-opacity': 0.2, 'width':20, 'height':20},
            },                   
        ]
...
body_layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dcc.Markdown('Filter nodes by these criteria:'),
...
def highlight_cites(usr_min_cites, usr_journals_list):

    node_list = filter_node_data(min_conns=usr_min_cites, journals=usr_journals_list, date_filter=None)
...
def displayTapNodeData(datalist):
